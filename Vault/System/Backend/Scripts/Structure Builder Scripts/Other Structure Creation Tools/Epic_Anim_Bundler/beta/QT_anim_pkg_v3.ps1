...
$trayIcon.Text = "Anim Package Engine"
    ...
    if (Test-Path $path) {
        ...
        if ($item.PSIsContainer) {
            ...
            if ($baseName.StartsWith("SIB")) {
                ...
                if ($stvFolder) {
                    Get-ChildItem -Path $item.FullName -Recurse | ForEach-Object {
                    ...
                    Get-ChildItem -Path $stvFolder.FullName -Recurse -File | Where-Object { $_.Name -like "*SIB*" } | ForEach-Object {
                        $newName = $_.Name -replace "SIB", "STV"
